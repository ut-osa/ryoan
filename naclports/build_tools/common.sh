...
readonly RYOAN_ROOT=${NACL_PACKAGES_OUT}/root
readonly RYOAN_HOST_PYROOT=${NACL_PACKAGES_BUILD}/python3-host/install_host
readonly RYOAN_HOST_PYBUILD=${NACL_PACKAGES_BUILD}/python3-host/build_host
readonly RYOAN_HOST_PYTHON=${RYOAN_HOST_PYROOT}/bin/python3.5
readonly RYOAN_SITE_PACKAGES="lib/python3.5/site-packages/"
readonly RYOAN_DEST_PYROOT=${RYOAN_ROOT}
...
GetRevision() {
  ...
  FULL_REVISION="trunk-812-g6e2f030"
...
RyoanPythonModuleBuildStep() {
  SetupCrossEnvironment
  Banner "Build ${PACKAGE_NAME} python module"
  ChangeDir "${SRC_DIR}"
  LogExecute rm -rf build dist
  MakeDir "${RYOAN_DEST_PYROOT}/${SITE_PACKAGES}"
  export PYTHONPATH="${RYOAN_HOST_PYROOT}/${RYOAN_SITE_PACKAGES}"
  export PYTHONPATH="${PYTHONPATH}:${RYOAN_DEST_PYROOT}/${RYOAN_SITE_PACKAGES}"
  export NACL_PORT_BUILD=${1:-dest}
  export NACL_BUILD_TREE=${RYOAN_DEST_PYROOT}
  export CFLAGS="${NACLPORTS_CPPFLAGS} ${NACLPORTS_CFLAGS}"
  export CXXFLAGS="${NACLPORTS_CPPFLAGS} ${NACLPORTS_CXXFLAGS}"
  export LDFLAGS="${NACLPORTS_LDFLAGS} ${NACLPORTS_LIBS}"
  export LIBS="${NACLPORTS_LIBS}"
  LogExecute "${RYOAN_HOST_PYTHON}" setup.py ${NACL_PYSETUP_ARGS} \
    install "--prefix=${RYOAN_DEST_PYROOT}"
...
RyoanPythonModuleInstallStep() {
  SetupCrossEnvironment
  ${AR} cr ${DEST_PYTHON_OBJS}/${PACKAGE_NAME}.a ${DEST_PYTHON_OBJS}/${PACKAGE_NAME}/*.o
  ${RANLIB} ${DEST_PYTHON_OBJS}/${PACKAGE_NAME}.a
}
...
LIB_PATH_DEFAULT=${NACL_SDK_LIBDIR}:${NACLPORTS_LIBDIR}:${INSTALL_DIR}/${PREFIX}/lib
