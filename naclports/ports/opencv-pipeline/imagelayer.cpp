...
const char* prog_name = "LAYER_IMAGE";

using namespace std;
using namespace cv;

bool do_layer(Mat base, Mat top, const char* dest){
    Mat result(base);
    Mat formatted;
    Size size(base.rows, base.cols);

    resize(top, formatted, size);
    addWeighted(base, 0.5, formatted, 0.5, 0.0, result);
        ...
        Mat& image_top, const char* dest, work_ctx ctx=NULL)
{
    ...
    if(! do_layer(image, image_top, dest)){
        return -1;
    }
