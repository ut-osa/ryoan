...
bool
FVector::save(int fd) const
{
  int i = size();
  const VFloat *d = rep()->data;
  if (write(fd, (const char*)&i, sizeof(int)) < (ssize_t)sizeof(int))
    return false;
  if (write(fd, (const char*)d, sizeof(VFloat)*i) < (ssize_t)sizeof(VFloat)*i)
    return false;
  return true;
}

char*
FVector::save(char *buf, int len) const
{
  int i = size();
  const VFloat *d = rep()->data;
  if (len < (ssize_t)(sizeof(int) + sizeof(VFloat)*i)) {
    return NULL;
  }
  *(int*)buf = i;
  buf += sizeof(int);
  memcpy(buf, (const char*)d, sizeof(VFloat)*i);
  return buf + sizeof(VFloat)*i;
}
...
FVector::load(int fd)
{
  int i = 0;
  clear();
  if (read(fd, (char*)&i, sizeof(int)) < (ssize_t)sizeof(int))
    return false;
  if (i<0)
    return false;
  resize(i);
  VFloat *d = rep()->data;
  if (read(fd, (char*)d, sizeof(VFloat)*i) < (ssize_t)sizeof(VFloat)*i)
    return false;
  return true;
}

char *
FVector::load(char *buf, int len)
{
  int i = 0;
  clear();
  if (len < (ssize_t)sizeof(int))
    return NULL;
  i = *(int*)buf;
  buf += sizeof(int);
  if (i<0)
  ...
  resize(i);
  VFloat *d = rep()->data;
  memcpy((char*)d, buf, sizeof(VFloat)*i);
  return buf + sizeof(VFloat)*i;
}
...
#endif
  VFloat sum = 0.0;
#if defined(__GNUC__) && defined(__SSE2__) && !defined(__APPLE__) && !defined(_NEWLIB_VERSION)
  typedef float v4sf __attribute__ ((vector_size (16)));
  if (sizeof(VFloat) == 4 && m >= 16)
    {
