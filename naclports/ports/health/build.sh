...
ConfigureStep() {
    ...
    LogExecute cp -rf ${START_DIR}/svmsgd/* ${SRC_DIR}
    (BuildHost)
    cd ${BUILD_DIR}
    EXTRA_CMAKE_ARGS=" -DEXEEXT:STRING=.nexe -DNACL_SYS_LIB=nacl_sys_private -DPTHREAD_LIB=pthread_private -DGLIBC-COMPAT=glibc-compat -DC++=c++"
