...
static
json_value *jsonify_WorkSpec(WorkSpec *work)
{
    json_value *val;
...
WorkSpec *fitting_to_workspec(FittingSpec *fit, FittingSpec *id_map[],
        int from_pred, int to_pred, int from_succ, int to_succ)
{
    WorkSpec *work;
    int i = 0;
    ...
    if (from_pred > 0) {
        ...
        work->channels[i]->in_fd = from_pred,
        work->channels[i]->out_fd = to_pred;
    ...
    if (to_succ > 0) {
        work->channels[i] = malloc(sizeof(ChannelDesc));
        work->channels[i]->other_path =
            alloc_string(id_map[fit->out_ids[0]]->argv[0]);
        work->channels[i]->in_fd = from_succ,
        work->channels[i]->out_fd = to_succ;
