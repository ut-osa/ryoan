...
BuildHost() {
  ...
  make clean || true
  LogExecute ./config \
    --prefix=${HOST_INSTALL_DIR} no-asm no-hw \
    no-krb5 shared -D_GNU_SOURCE -L${HOST_INSTALL_DIR}/lib \
    -Wl,-rpath=${HOST_INSTALL_DIR}/lib -I${HOST_INSTALL_DIR}/include
  LogExecute make -j${OS_JOBS}
  LogExecute make install_sw
}
