		...
		ALLOCATE_STACK_PARMS)
{
  ...
  if (__builtin_expect (attr->flags & ATTR_FLAG_STACKADDR, 0))
    {
      if (in_sgx) DIE("Pre allocated stack?!? Really Man?");
      uintptr_t adj;
      ...
      if (pd == NULL)
	{
...

      {
          uint64_t full_size;
          if (in_sgx) {
              /* just make damn sure we have enough space to put our data
                 where we expect it to be */
              full_size = size + __tls_size + PAGE_SIZE;
          }
          else
              full_size = size;
          mem = mmap (NULL, full_size, prot,
