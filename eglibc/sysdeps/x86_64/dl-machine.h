...
_start:\n\
    movq $0, %r15\n\
    movq _in_sgx@GOTPCREL(%rip), %rdi\n\
    movq (%rdi), %rdi\n\
    test %rdi, %rdi\n\
    jne _do_sgx_entry\n\
    cmp $"STR(SGX_KEY)", %rsi\n /* hack so that this still works w/o sgx*/\
    je  do_sgx_entry\n\
    # normal start\n\
    movq %rsp, %rdi\n\
    call _dl_start\n\
...
do_sgx_entry:\n\
    ...
    movq (%rbx), %rsp\n\
    movq 8(%rbx), %rbp\n\
    movq _tls_size@GOTPCREL(%rip), %r13\n\
    movq (%r13), %r14\n\
    test %r14, %r14\n\
    je   sgx_init\n\
    #Save a pointer for later\n\
    subq $8, %r14\n\
    movq %rbx, %fs:(%r14)\n\
    movq %rax, %rdi\n\
    movq %rbx, %rsi\n\
    movq %rcx, %rdx\n\
 # switch to sigalt stack if we have it\n\
    movq 56(%rbx), %r12\n\
    test %r12, %r12\n\
    je dont_change_stack\n\
    xchgq %r12, %rsp\n\
    jmp do_signal\n\
dont_change_stack:\n\
    movq %rsp, %r12\n\
...
do_signal:\n\
    ...
    movq check_exception@GOTPCREL(%rip), %rax\n\
    call *%rax\n\
    popq %r11\n\
 # %r12 has the old stack val\n\
    popq %rsp\n\
    ret\n\
sgx_init:\n\
    test %r15, %r15\n\
    jne test_fail\n\
    # /* we must be first! save the tls size! */\n\
    movq 24(%rbx), %r14\n\
    movq %r14, (%r13)\n\
    subq $8, %r14\n\
    movq %rbx, %fs:(%r14)\n\
