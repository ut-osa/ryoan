...
struct sgx_man_info;
int do_sgx_man_init(uint64_t test_val, struct sgx_man_info *);

ElfW(Addr) __attribute_used__ internal_function
_dl_start (void *arg, uint64_t sgx_test_val, struct sgx_man_info *sgx_info)
{
#ifdef DONT_USE_BOOTSTRAP_MAP
...
# endif
  ...
  {
    /* Init Enclave Management asap */
    do_sgx_man_init(sgx_test_val, sgx_info);
#ifdef DONT_USE_BOOTSTRAP_MAP
...
\n\
  ...
  if (in_sgx) {
      ...
      void (*vdso_init)(void *) = _dl_vdso_vsym ("__vdso_init", &linux26);
      vdso_init(syscalldb);
  }
...
void stop_if_sgx(void)
{
    if (in_sgx) __builtin_trap();
}
