...
int32_t NaClSysMmapTainted(struct NaClApp        *nap,
                           void                  *start,
                           size_t                length,
                           int                   prot,
                           int                   flags)
{
  ...
  allowed_flags = (NACL_ABI_MAP_PRIVATE | NACL_ABI_MAP_ANONYMOUS);
  ASSERT(start == NULL);
  ASSERT(0 == (flags & ~allowed_flags));
  ASSERT(prot == (NACL_ABI_PROT_READ | NACL_ABI_PROT_WRITE));
   ...
   * We pick one that's best for the system.
   */
  usraddr = NaClGetMallocPoolChunk(&nap->malloc_pool, length);
  map_result = usraddr;
...
int32_t NaClSysMmap(struct NaClAppThread  *natp,
  ...
  if (NaClAppIsConfined()) {
      return NaClSysMmapTainted(nap, (void *)0, length, prot, flags);
  } else {
...
int32_t NaClSysMunmap(struct NaClAppThread  *natp,
  ...
  if (NaClAppIsConfined()) {
      //NaClLog(LOG_ERROR, "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n");
      retval = NaClFreeMallocPoolChunk(&nap->malloc_pool, start, length);
      goto cleanup;
  }
...
int32_t NaClSysMprotect(struct NaClAppThread  *natp,
  ...
  if (NaClAppIsConfined())
  {
     NaClLog(LOG_FATAL, "CONFINED APP TRIED TO MPROTECT");
  }
