...
struct NaClApp {
  ...
  struct DynArray           read_buffer_tbl;  /* Read buffer pointers */

  struct DynArray           write_buffer_tbl; /* Write buffer pointers */

  struct NaClSemaphore      request_sem;      /* sem for wait for request */

  struct NaClSemaphore      file_buffer_sem;
  struct DynArray           file_buffer_tbl;   /* File buffer pointers */
  uint32_t                  max_host_fd;

  struct redir*             redir_read_queue;  /* list of read side host fd:nacl app fd */
  struct redir*             redir_write_queue; /* list of write side host fd:nacl app fd */

  struct cfuhash_table      *file_name_tbl;    /* Hash table maps from path to host desc */ 

  struct NaClMutex          defer_desc_mu;
  struct DynArray           defer_tbl; /* safe the descriptor not be closed */
  int                       defer_count;
  ...
  uint8_t*                  nonce;
  uint32_t                  total_bytes_for_unit;
  char                      *fake_app_label;
  uint8_t                   get_user_input;
  uint8_t                   output_to_user;
  uint8_t                   no_enc;
  ...
  /*
   * NaClVmmap to manage sgx_malloc_pool, we allocate a fake region in
   * mem_map so that NaCl doesn't step on us
   */
  struct SgxMallocPool malloc_pool;
  unsigned long requests_per_reset;
  unsigned long ryoan_stop_mode;
...
/*
 * Versions that are called while already holding the semaphore
 */
...
int32_t NaClAppSetFileBufferAvailSem(struct NaClApp        *nap,
