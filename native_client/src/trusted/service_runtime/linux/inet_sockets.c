...
#include <string.h>
#include <stdbool.h>
#include "native_client/src/trusted/service_runtime/linux/inet_sockets.h"       /* Declares functions defined here */
                  ...
                  bool doListen, int backlog)
{
  ...
  if (rp == NULL) {
    NaClLog(LOG_FATAL, "bind: %s\n", strerror(errno));
    return -1;
  } else {
    if (makeFdNonBlocking(sfd) == -1) {
      return -1;
    }

    if (doListen) {
      if (listen(sfd, backlog) == -1) {
        NaClLog(LOG_FATAL, "listen: %s\n", strerror(errno));
        return -1;
      }
    }
    if (addrlen != NULL)
