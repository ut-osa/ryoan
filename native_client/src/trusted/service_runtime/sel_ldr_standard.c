...
NaClErrorCode NaClAppLoadFile(struct NaClDesc *ndp,
                              struct NaClApp *nap) {
  /*  XXX(thunt): using NACL_ENAVLE_ASLR will eventually succeed if we
      fail to allocate but causes a large number of rtries so we just turn
      it off here*/
  return NaClAppLoadFileAslr(ndp, nap, NACL_DISABLE_ASLR/*NACL_ENABLE_ASLR*/);
}
