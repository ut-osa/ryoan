...
static inline bool entry_is_valid(uintptr_t e) { return e > HTABLE_DELETED; }
...
bool htable_init_sized(struct htable *ht,
                       size_t (*rehash)(const void *, void *), void *priv,
                       size_t expect) {
...
void htable_clear(struct htable *ht) {
  if (ht->table != &ht->perfect_bit) free((void *)ht->table);
...
void *htable_firstval(const struct htable *ht, struct htable_iter *i,
                      size_t hash) {
  i->off = hash_bucket(ht, hash);
  return htable_val(ht, i, hash, ht->perfect_bit);
}

void *htable_nextval(const struct htable *ht, struct htable_iter *i,
                     size_t hash) {
  i->off = (i->off + 1) & ((1 << ht->bits) - 1);
...
static COLD void update_common(struct htable *ht, const void *p) {
  ...
  if (ht->elems == 0) {
    ...
    for (i = sizeof(uintptr_t) * CHAR_BIT - 1; i > 0; i--) {
      if ((uintptr_t)p & ((uintptr_t)1 << i)) break;
    }
  ...
  for (i = 0; i < (size_t)1 << ht->bits; i++) {
    if (!entry_is_valid(ht->table[i])) continue;
...
bool htable_add(struct htable *ht, size_t hash, const void *p) {
  if (ht->elems + 1 > ht->max && !double_table(ht)) return false;
  if (ht->elems + 1 + ht->deleted > ht->max_with_deleted) rehash_table(ht);
  assert(p);
  if (((uintptr_t)p & ht->common_mask) != ht->common_bits) update_common(ht, p);
