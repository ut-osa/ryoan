...
const char const *Channel_Type_str[] = {
    "PLAIN_CHANNEL", "DH_CHANNEL",
};
const char const *Flow_Dir_str[] = {
    "IN", "OUT",
};

static inline int __test(const char *str, const char const *arr[],
                         size_t size) {
  int i;
  for (i = 0; i < size; ++i) {
...
inline const char *flow_dir_str(Flow_Dir type) { return Flow_Dir_str[type]; }
...
static void _free_channel(channel_t *chan) { free(chan); }

static channel_t *build_channel(chan_read_op chan_read,
                                chan_write_op chan_write,
                                chan_free_op chan_free,
                                chan_ready_op chan_ready,
                                chan_get_in_fd get_in_fd) {
  ...
  *(chan_get_in_fd *)&chan->get_in_fd = get_in_fd;
  chan->opaque = NULL;
...
/********* plaintext channels ********/
static ssize_t plain_text_read(channel_t *self, void *buff, size_t count) {
  return read(((int *)self->opaque)[0], buff, count);
}

static ssize_t plain_text_write(channel_t *self, const void *buff,
                                size_t count) {
...
static int plain_text_ready(channel_t *self, Flow_Dir dir) {
  struct pollfd fds = {0};
...
static int *plain_text_get_in_fd(channel_t *self) {
  return ((int *)self->opaque);
}
...
channel_t *build_plain_text_channel(int in, int out) {
  ...
  int *fds;
  channel_t *chan =
      build_channel(plain_text_read, plain_text_write, free_plain_text_channel,
                    plain_text_ready, plain_text_get_in_fd);
...
static DH *create_dh() {
  ...
  if (DH_generate_key(dh) != 1) {
    char buf[100];
    eprintf(ERR_error_string(ERR_get_error(), buf));
...
static ssize_t d_h_read(channel_t *self, void *buff, size_t count) {
...
static ssize_t d_h_write(channel_t *self, const void *buff, size_t count) {
  BIO *out = ((dh_context *)self->opaque)->out;
  ssize_t ret = BIO_write(out, buff, count);
  return ret;
}

static int hash_report_send(int fd, const void *buffer, size_t size,
                            SGX_identity_t *other_id) {
...
static int recieve_report_verify(int fd, const void *buffer, size_t size) {
  hash_t hash;
  SGX_report report;
...
static unsigned char *do_dh_key_exchange(int in, int out,
                                         SGX_identity_t *other_id) {
  DH *dh;
...
static int exchange_mrenclave(int in, int out, SGX_identity_t *id) {
  SGX_userdata_t userdata __align(128);
  SGX_targetinfo targetinfo __align(128);
  SGX_report report;
  if (_ereport(&report, &userdata, &targetinfo)) return -1;
  ...
  /*recieve theirs*/
  if (read(in, id, sizeof(SGX_identity_t)) != sizeof(SGX_identity_t)) return -1;

  return 0;
}

static void free_d_h_channel(channel_t *chan, int close) {
...
static int dh_ready(channel_t *self, Flow_Dir dir) {
...
channel_t *build_d_h_channel(int in, int out) {
  ...
  channel_t *chan =
      build_channel(d_h_read, d_h_write, free_d_h_channel, dh_ready, NULL);
  if (!chan) {
    return NULL;
  }
