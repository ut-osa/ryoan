...
#define LQUEUE(etype, link) \
  TCON_WRAP(struct lqueue_, TCON_CONTAINER(canary, etype, link))
 ...
 *  lqueue_init(qp);
 */
#define lqueue_init(q_) (lqueue_init_(tcon_unwrap(q_), NULL))
 ...
 */
#define lqueue_empty(q_) lqueue_empty_(tcon_unwrap(q_))
 ...
 */
#define lqueue_front(q_) lqueue_entry((q_), lqueue_front_(tcon_unwrap(q_)))
 ...
 */
#define lqueue_back(q_) lqueue_entry((q_), lqueue_back_(tcon_unwrap(q_)))
 ...
 */
#define lqueue_dequeue(q_) lqueue_entry((q_), lqueue_dequeue_(tcon_unwrap(q_)))
static inline struct lqueue_link *lqueue_dequeue_(struct lqueue_ *q) {
  struct lqueue_link *front;

  if (lqueue_empty_(q)) return NULL;
