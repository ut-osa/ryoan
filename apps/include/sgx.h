...
typedef struct SGX_identity_t { uint8_t payload[32]; } SGX_identity_t;

typedef struct SGX_attributes_t { uint8_t payload[16]; } SGX_attributes_t;
...
static inline int SecInfo_check_reserved(SecInfo *secinfo) {
  ...
  for (i = 0; i < sizeof(secinfo->reserved); ++i) {
    if (secinfo->reserved[i]) return 1;
  }
  return 0;
}

typedef struct SGX_key_t { uint8_t payload[16]; } SGX_key_t;

typedef struct SGX_userdata_t { uint8_t payload[64]; } SGX_userdata_t;

typedef struct cmac_t { uint8_t payload[16]; } cmac_t;
